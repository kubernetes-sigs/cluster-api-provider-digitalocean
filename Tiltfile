...
def capdo():
    ...
    if settings.get("extra_args"):
        do_extra_args = settings.get("extra_args").get("do")
        if do_extra_args:
            yaml_dict = decode_yaml_stream(yaml)
            append_arg_for_container_in_deployment(yaml_dict, "capdo-controller-manager", "capdo-system", "cluster-api-do-controller", do_extra_args)
    ...
    docker_build(
        ref = "gcr.io/k8s-staging-cluster-api-do/cluster-api-do-controller",
        context = "./.tiltbuild/",

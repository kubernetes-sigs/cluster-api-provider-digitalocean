...
# Machine settings.
export CONTROL_PLANE_MACHINE_TYPE="${CONTROL_PLANE_MACHINE_TYPE:-s-2vcpu-2gb}"
export NODE_MACHINE_TYPE="${NODE_MACHINE_TYPE:-s-2vcpu-2gb}"
export MACHINE_IMAGE="${MACHINE_IMAGE:?}"
export SSH_KEY_PATH="${SSH_KEY_PATH:-${HOME}/.ssh/id_rsa.pub}"
export SSH_KEY_FINGERPRINT=$(ssh-keygen -E md5 -lf "${SSH_KEY_PATH}" | awk '{ print $2 }' | cut -c 5-)

# Outputs.
COMPONENTS_CLUSTER_API_GENERATED_FILE=${SOURCE_DIR}/core-components/cluster-api-components.yaml
COMPONENTS_KUBEADM_GENERATED_FILE=${SOURCE_DIR}/core-components/bootstrap-components.yaml
COMPONENTS_DO_GENERATED_FILE=${SOURCE_DIR}/provider-components/provider-components-digitalocean.yaml

CORE_COMPONENTS_GENERATED_FILE=${OUTPUT_DIR}/core-components.yaml
...
while test $# -gt 0; do
        case "$1" in
          -h|--help)
            echo "$SCRIPT - generates input yaml files for Cluster API on digitalocean"
            echo " "
            echo "$SCRIPT [options]"
            echo " "
            echo "options:"
...
# Generate DO Credentials.
DO_B64ENCODED_CREDENTIALS="$(echo -n "${DIGITALOCEAN_ACCESS_TOKEN:?}" | base64 | tr -d '\n')"
...
kustomize build "${SOURCE_DIR}/core-components" | envsubst > "${CORE_COMPONENTS_GENERATED_FILE}"
echo "Generated ${CORE_COMPONENTS_GENERATED_FILE}"

# Generate DO Infrastructure Provider components file.
kustomize build "${SOURCE_DIR}/../config/default" | envsubst > "${COMPONENTS_DO_GENERATED_FILE}"
echo "Generated ${COMPONENTS_DO_GENERATED_FILE}"
...
echo "WARNING: ${PROVIDER_COMPONENTS_GENERATED_FILE} includes DO credentials"
